 ...
 */
  ...
  /**
  * 在Vue.prototype._init中
  * parent => this.constructor.options
  * child => options
  */ 
export function mergeOptions (parent, child, vm) {
  // 将child 中的components重新组装，最先的child中的components放入之后新组装的components.options里面
  // newOptions = mergeOptions(Vue.options, oldOptions)
  guardComponents(child)

  // 对props进行一些修正和转换
  // child.props = ['value'] => child.props = {value: null}
  // child.props = [{name: 'value'}] => child.props = {value: {name: 'value'}}
  // child.props = {value: function() {console.log(1)}} => child.props = {value: {type: function() {console.log(1)}}}
  guardProps(child)
  if (process.env.NODE_ENV !== 'production') {
  ...
  // 对组件所有属性进行相应的合并处理
  // 如options.data, options.created, options.methods等
